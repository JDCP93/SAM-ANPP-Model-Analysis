SampleFunction_P = function(Site,Model="Obs",block,Nlag=3,SampleLength){
  ...
  # If the modelling has already been run, let us know and don't run again
    if (file.exists(paste0(Site,"_P_",StartYear,"_",SampleLength,"SL_",Model,"_pos_",Nlag,"_",max(block),".Rdata"))){
      print(paste0("Sampled SAM_P output of length ",
                   SampleLength,
                   " starting from ",
                   StartYear,
                   " already exists for ",
      ...
      SampleSAM_P(Site,ANPP,PPT,Nlag,block,prior=FALSE,Model,SampleLength,StartYear)
