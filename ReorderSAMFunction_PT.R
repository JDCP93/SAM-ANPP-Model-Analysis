ReorderSAM_PT <- function(Site,ANPP,Precip,Tair,Nlag,block,prior=FALSE,model="Obs"){
   ...
   if (prior==TRUE){
      Data$ANPP = NULL
      print(paste0("Running prior SAM_PT model for ",Site," with lag ",Nlag," and ",Data$Nblocks, " blocks, using reordered ",model," data"))
   }else{print(paste0("Running posterior SAM_PT model for ",Site," with lag ",Nlag," and ",Data$Nblocks, " blocks, using reordered ",model," data"))}
   ...
   if (prior==TRUE){ 
      ...
      name = paste0(Site,"_PT_Reorder_",model,"_pri_",Nlag,"_",Data$Nblocks)  
      assign(name,output)
   ...
   # for the posteriors calculate the performance metrics and output
      ...
      # Write output file
      name = paste0(Site,"_PT_Reorder_",model,"_pos_",Nlag,"_",Data$Nblocks)
      assign(name,output)
      save(list=c(name),file=paste0(name,".Rdata"))
